...
class GaussianModel:
    ...
    def get_level_1_features(self, time_idx):
        return self._level_1_features_dc[:, time_idx, :]

    def get_all_features(self, time_idx):
        return torch.cat((self._features_dc, self.get_level_1_features(time_idx)), dim=0)

    def get_level_1_features_all_t(self):
    ...
    def create_another_level(
        ...
        **kwargs,
        ...
        level_1_fused_color = (
            torch.zeros((self._level_1_parent_idx.shape[0], 120, self._features_dc.shape[1])).float().cuda()
        )
    ...
    def save_ply(self, path):
        ...
        f_dc_t = self._features_dc.reshape(-1, 1, self._features_dc.shape[1])
        f_dc_t = f_dc_t.repeat(1, 120, 1)
        f_dc_t = f_dc_t.detach().cpu().numpy()
        ...
        if self._level_1_parent_idx.shape[0] > 0:
            ...
            level_1_f_dc_t = self._level_1_features_dc.detach().cpu().numpy()
            ...
            all_f_dc_t = np.concatenate((f_dc_t, level_1_f_dc_t), axis=0)
    ...
    def densify_and_split_level_1(self, grads, grad_threshold, scene_extent, max_timestamp=1.0, N=2):
        ...
        new_features_dc = self._level_1_features_dc[selected_pts_mask].repeat(N, 1, 1)
    ...
    def post_prune_level_1(self, min_opacity, extent, max_screen_size, prune_min_color=None, prune_max_color=None):
        ...
        if prune_min_color is not None and isinstance(prune_min_color, float):
            color_mask = self.get_level_1_features_all_t < prune_min_color
            color_mask = color_mask.all(dim=1)
            prune_mask_color = color_mask.squeeze()
        ...
        if prune_max_color is not None and isinstance(prune_max_color, float):
            color_mask = self.get_level_1_features_all_t > prune_max_color
            color_mask = color_mask.all(dim=1)
            prune_mask_color = color_mask.squeeze()
