...
def convert(
    AABB_SCALE, SKIP_EARLY, IMAGE_FOLDER, TEXT_FOLDER, OUT_PATH, totp=-1, totw=-1, avglen=-1, rMat=np.array([])
):
...
def createNerf(path, hires=False):
    ...
    totp, totw, avglen, rMat = convert(
        AABB_SCALE, SKIP_EARLY, IMAGE_FOLDER, TEXT_FOLDER, OUT_PATH, -1, -1, -1, np.array([])
    )
    ...
    OUT_PATH = os.path.join(os.path.join(colmappath, "stereo"), "transforms.json")
    totp, totw, avglen, rMat = convert(
        AABB_SCALE, SKIP_EARLY, IMAGE_FOLDER, TEXT_FOLDER, OUT_PATH, totp, totw, avglen, rMat
    )
