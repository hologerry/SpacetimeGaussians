...
def train_lite_act_two_sp_level_couple_transp_linsindel_trbfs(
    viewpoint_camera,
    gm: GaussianModel,
    pipe,
    ...
    **kwargs,
    ...
    if level == 0:
        ...
        level_1_delta_sin_a = gm.get_level_1_delta_sin_a
        level_1_delta_sin_omega = gm.get_level_1_delta_sin_omega
        level_1_delta_sin_phi = gm.get_level_1_delta_sin_phi

        level_1_parent_idx = level_1_parent_idx.squeeze(1)

        level_1_parent_means3D = level_0_cur_means3D[level_1_parent_idx]

        level_1_delta_means3D_sin_0 = level_1_delta_sin_a[:, 0:3] * torch.sin(
            2
            * math.pi
            * (level_1_delta_sin_omega[:, 0:3] * viewpoint_camera.timestamp + level_1_delta_sin_phi[:, 0:3])
        )
        # level_1_delta_means3D_sin_1 = level_1_delta_sin_a[:, 3:6] * torch.sin(
        #     2
        #     * math.pi
        #     * (level_1_delta_sin_omega[:, 3:6] * viewpoint_camera.timestamp + level_1_delta_sin_phi[:, 3:6])
        # )
        # level_1_delta_means3D_sin_2 = level_1_delta_sin_a[:, 6:9] * torch.sin(
        #     2
        #     * math.pi
        #     * (level_1_delta_sin_omega[:, 6:9] * viewpoint_camera.timestamp + level_1_delta_sin_phi[:, 6:9])
        # )
        level_1_delta_means3D = level_1_delta_means3D_sin_0 # + level_1_delta_means3D_sin_1 + level_1_delta_means3D_sin_2
...
def test_lite_act_two_sp_level_couple_transp_linsindel_trbfs_vis(
    viewpoint_camera,
    gm: GaussianModel,
    pipe,
    bg_color: torch.Tensor,
    ...
    **kwargs,
    ...
    level_0_delta_sin_a_dummy = torch.zeros((n_level_0_points, 9), dtype=torch.float32, device="cuda")
    level_0_delta_sin_omega_dummy = torch.zeros((n_level_0_points, 9), dtype=torch.float32, device="cuda")
    level_0_delta_sin_phi_dummy = torch.zeros((n_level_0_points, 9), dtype=torch.float32, device="cuda")
    ...
    if level == 0:
        ...
        delta_sin_a = level_0_delta_sin_a_dummy
        delta_sin_omega = level_0_delta_sin_omega_dummy
        delta_sin_phi = level_0_delta_sin_phi_dummy
        ...
        level_1_means3D = level_1_parent_means3D
        ...
        # 2 * π * a * w * cos(2 * π * (w * x + p))
        level_1_delta_velocities3D_sin_0 = (
            level_1_delta_sin_a[:, 0:3]
            * 2
            * math.pi
            * level_1_delta_sin_omega[:, 0:3]
            * torch.cos(
                2
                * math.pi
                * (level_1_delta_sin_omega[:, 0:3] * viewpoint_camera.timestamp + level_1_delta_sin_phi[:, 0:3])
        ...
        # level_1_delta_velocities3D_sin_1 = (
        #     level_1_delta_sin_a[:, 3:6]
        #     * 2
        #     * math.pi
        #     * level_1_delta_sin_omega[:, 3:6]
        #     * torch.cos(
        #         2
        #         * math.pi
        #         * (level_1_delta_sin_omega[:, 3:6] * viewpoint_camera.timestamp + level_1_delta_sin_phi[:, 3:6])
        #     )
        # )
        # level_1_delta_velocities3D_sin_2 = (
        #     level_1_delta_sin_a[:, 6:9]
        #     * 2
        #     * math.pi
        #     * level_1_delta_sin_omega[:, 6:9]
        #     * torch.cos(
        #         2
        #         * math.pi
        #         * (level_1_delta_sin_omega[:, 6:9] * viewpoint_camera.timestamp + level_1_delta_sin_phi[:, 6:9])
        ...
        level_1_delta_velocities3D = (
            level_1_delta_velocities3D_sin_0 #+ level_1_delta_velocities3D_sin_1 + level_1_delta_velocities3D_sin_2
        )
        ...
        if transp_level_0:
            ...
            delta_sin_a = level_1_delta_sin_a
            delta_sin_omega = level_1_delta_sin_omega
            delta_sin_phi = level_1_delta_sin_phi
            ...
            delta_sin_a = torch.cat((level_0_delta_sin_a_dummy, level_1_delta_sin_a), dim=0)
            delta_sin_omega = torch.cat((level_0_delta_sin_omega_dummy, level_1_delta_sin_omega), dim=0)
            delta_sin_phi = torch.cat((level_0_delta_sin_phi_dummy, level_1_delta_sin_phi), dim=0)
    ...
    output_dict = {
        ...
        "delta_sin_a": delta_sin_a,
        "delta_sin_omega": delta_sin_omega,
        "delta_sin_phi": delta_sin_phi,
    }

    if level == 1:
        output_dict["level_1_parent_means3D"] = level_1_parent_means3D
        output_dict["level_1_parent_velocity3D"] = level_1_parent_velocity3D
        output_dict["level_1_delta_means3D"] = level_1_delta_means3D
        output_dict["level_1_delta_velocities3D"] = level_1_delta_velocities3D
