...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        self._delta_sin_a = torch.zeros((fused_point_cloud.shape[0], 3), device="cuda")
        self._delta_sin_omega = torch.zeros((fused_point_cloud.shape[0], 3), device="cuda")
        self._delta_sin_phi = torch.zeros((fused_point_cloud.shape[0], 3), device="cuda")
    ...
    def create_another_level(
        ...
        **kwargs,
        ...
        level_1_delta_sin_a = (
            torch.zeros((level_1_total_parent_idx.shape[0], 3), device="cuda") + new_pts_init_delta_sin_a
        )
        self._level_1_delta_sin_a = nn.Parameter(level_1_delta_sin_a.requires_grad_(True))
        level_1_delta_sin_omega = (
            torch.zeros((level_1_total_parent_idx.shape[0], 3), device="cuda") + new_pts_init_delta_sin_omega
        )
        self._level_1_delta_sin_omega = nn.Parameter(level_1_delta_sin_omega.requires_grad_(True))
        level_1_delta_sin_phi = (
            torch.zeros((level_1_total_parent_idx.shape[0], 3), device="cuda") + new_pts_init_delta_sin_phi
        )
    ...
    def save_ply(self, path):
        ...
        delta_sin_a_dummy = np.zeros((xyz.shape[0], 3))
        delta_sin_omega_dummy = np.zeros((xyz.shape[0], 3))
        delta_sin_phi_dummy = np.zeros((xyz.shape[0], 3))
