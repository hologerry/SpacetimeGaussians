...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int, rgb_function="rgbv1"):
        ...
        self._level_1_xyz = torch.empty(0)
    ...
    def create_another_level(
        ...
        n_parent_level = self.get_xyz.shape[0]
        ...
        level_1_total_xyz = torch.cat(level_1_total_xyz, dim=0).float().cuda()
        level_1_total_times = torch.cat(level_1_total_times, dim=0).float().cuda()

        self._level_1_xyz = nn.Parameter(level_1_total_xyz.requires_grad_(True))
        ...
        level_1_rots = torch.zeros((level_1_total_xyz.shape[0], 4), device="cuda")
        level_1_rots[:, 0] = 1.0
        ...
        level_1_omega = torch.zeros((level_1_total_xyz.shape[0], 4), device="cuda")
        ...
        level_1_motion = torch.zeros((level_1_total_xyz.shape[0], 9), device="cuda")
        self._level_1_motion = nn.Parameter(level_1_motion.requires_grad_(True))
        print(f"self._level_1_motion inited {self._level_1_motion}")

        self.level_1_max_radii2D = torch.zeros((level_1_total_xyz.shape[0]), device="cuda")
        ...
        level_1_trbf_scale = torch.ones((level_1_total_xyz.shape[0], 1), device="cuda")
    ...
    def level_1_training_setup(self, training_args):
        self.level_1_xyz_gradient_accum = torch.zeros((self.get_level_1_xyz.shape[0], 1), device="cuda")
        self.level_1_denom = torch.zeros((self.get_level_1_xyz.shape[0], 1), device="cuda")
        l = [
            {
    ...
    def save_ply(self, path):
        ...
        if self._level_1_xyz.shape[0] > 0:

            level_1_xyz = self._level_1_xyz.detach().cpu().numpy()
            level_1_normals = np.zeros_like(level_1_xyz)
            ...
            level_1_motion = self._level_1_motion.detach().cpu().numpy()
            level_1_omega = self._level_1_omega.detach().cpu().numpy()
            level_1_level = np.ones((level_1_xyz.shape[0], 1))

            all_xyz = np.concatenate((xyz, level_1_xyz), axis=0)
            ...
            all_motion = np.concatenate((motion, level_1_motion), axis=0)
    ...
    def load_ply(self, path):
        ...
        if np.sum(level_1_idx) > 0:
            self._level_1_xyz = nn.Parameter(
                torch.tensor(xyz[level_1_idx], dtype=torch.float, device="cuda").requires_grad_(True)
            )
            ...
            # self._level_1_features_rest = nn.Parameter(torch.tensor(features_extra, dtype=torch.float, device="cuda").requires_grad_(True))
            ...
            self._level_1_motion = nn.Parameter(
                torch.tensor(motion[level_1_idx], dtype=torch.float, device="cuda").requires_grad_(True)
            )
    ...
    def densification_postfix_level_1(
        ...
        self.level_1_xyz_gradient_accum = torch.zeros((self.get_level_1_xyz.shape[0], 1), device="cuda")
        self.level_1_denom = torch.zeros((self.get_level_1_xyz.shape[0], 1), device="cuda")
        self.level_1_max_radii2D = torch.zeros((self.get_level_1_xyz.shape[0]), device="cuda")
    ...
    def densify_and_split_level_1(self, grads, grad_threshold, scene_extent, max_timestamp=1.0, N=2):
        n_init_points = self.get_level_1_xyz.shape[0]
        padded_grad = torch.zeros((n_init_points), device="cuda")
        padded_grad[: grads.shape[0]] = grads.squeeze()
        selected_pts_mask = torch.where(padded_grad >= grad_threshold, True, False)
        selected_pts_mask = torch.logical_and(
        ...
        new_features_dc = self._level_1_features_dc[selected_pts_mask].repeat(N, 1)  # n,1,1 to n1
    ...
    def densify_and_clone_level_1(self, grads, grad_threshold, scene_extent, max_timestamp=1.0):
        ...
        new_xyz = self._level_1_xyz[selected_pts_mask]
    ...
    def add_densification_stats_level_1(self, viewspace_point_tensor, update_filter):
        # since in render, all points are used
        n_level_0_points = self.get_xyz.shape[0]
        level_1_update_filter = update_filter[n_level_0_points:]
        viewspace_point_tensor_grad = viewspace_point_tensor.grad
        level_1_viewspace_point_tensor_grad = viewspace_point_tensor_grad[n_level_0_points:]
