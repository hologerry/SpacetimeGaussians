...
def main():
    ...
    parser.add_argument(
        "--colmapPath", type=str, default=getColmapPath(), help="path to directory colmap.bat / colmap.bin directory"
    )
    parser.add_argument(
        "--quality",
        type=str,
    ...
    parser.add_argument(
        "--rc_path", type=str, required=False, help="path to rc dataset, containing bundle.out and images"
    )
    ...
    parser.add_argument(
        "--from_step", type=str, default="default", help="Run from this step to --to_step (or end if no to_step"
    )
    parser.add_argument(
        "--to_step",
        type=str,
    ...
    parser.add_argument(
        "--auto_recon_area", action="store_true", help="automatically set recon area (no user intervention)"
    )

    parser.add_argument(
        "--config_folder", type=str, default="default", help="folder containing configuration files; usually cwd"
    )
    ...
    parser.add_argument(
        "--no_refl", action="store_true", help="dont densify mesh, dont convert_sibr, dont create nerf (def: false)"
    )

    # needed to avoid parsing issue for passing arguments to next command (TODO)
    parser.add_argument(
        "--video_filename", type=str, default="default", help="full path of video file (internal argument; do not set)"
    )
    parser.add_argument(
        "--mesh_obj_filename",
        type=str,
    ...
    # Patch match stereo
    parser.add_argument(
        "--PatchMatchStereo.max_image_size", type=int, dest="patchMatchStereo_PatchMatchStereoDotMaxImageSize"
    )
    parser.add_argument(
        "--PatchMatchStereo.window_radius", type=int, dest="patchMatchStereo_PatchMatchStereoDotWindowRadius"
    )
    parser.add_argument(
        "--PatchMatchStereo.window_step", type=int, dest="patchMatchStereo_PatchMatchStereoDotWindowStep"
    )
    parser.add_argument(
        "--PatchMatchStereo.num_samples", type=int, dest="patchMatchStereo_PatchMatchStereoDotNumSamples"
    )
    parser.add_argument(
        "--PatchMatchStereo.num_iterations", type=int, dest="patchMatchStereo_PatchMatchStereoDotNumIterations"
    )
    parser.add_argument(
        "--PatchMatchStereo.geom_consistency", type=int, dest="patchMatchStereo_PatchMatchStereoDotGeomConsistency"
    )
    ...
    programs = {"colmap": {"path": getColmap(args["colmapPath"])}, "RC": {"path": getRCprocess()}}
