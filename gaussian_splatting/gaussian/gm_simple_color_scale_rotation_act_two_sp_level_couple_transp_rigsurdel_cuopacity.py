...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int, rgb_function="rgbv1"):
        ...
        self._level_1_opacity_a1 = torch.empty(0)
        self._level_1_opacity_c1 = torch.empty(0)
        self._level_1_opacity_a2 = torch.empty(0)
        self._level_1_opacity_c2 = torch.empty(0)
        self._level_1_opacity_a3 = torch.empty(0)
        self._level_1_opacity_c3 = torch.empty(0)
    ...
    @property
    def get_level_1_opacity_a1(self):
        return self._level_1_opacity_a1

    @property
    def get_level_1_opacity_c1(self):
        return self._level_1_opacity_c1

    @property
    def get_level_1_opacity_a2(self):
        return self._level_1_opacity_a2

    @property
    def get_level_1_opacity_c2(self):
        return self._level_1_opacity_c2

    @property
    def get_level_1_opacity_a3(self):
        return self._level_1_opacity_a3

    @property
    def get_level_1_opacity_c3(self):
        return self._level_1_opacity_c3

    @property
    def get_all_trbf_scale(self):
        return torch.cat((self._trbf_scale, torch.zeros((self._level_1_parent_idx.shape[0], 1), device="cuda")), dim=0)
    ...
    def create_another_level(
        ...
        **kwargs,
        ...
        level_1_total_sur_radius = []
        level_1_total_sur_azimuth = []
        level_1_total_sur_polar = []
        ...
        level_1_total_radius = torch.cat(level_1_total_sur_radius, dim=0)
        level_1_total_azimuth = torch.cat(level_1_total_sur_azimuth, dim=0)
        level_1_total_polar = torch.cat(level_1_total_sur_polar, dim=0)
        ...
        level_1_opacity_a1 = torch.zeros((level_1_total_parent_idx.shape[0], 1), device="cuda")
        self._level_1_opacity_a1 = nn.Parameter(level_1_opacity_a1.requires_grad_(True))
        print(f"self._level_1_opacity_a1 inited {self._level_1_opacity_a1}")
        level_1_opacity_c1 = torch.zeros((level_1_total_parent_idx.shape[0], 1), device="cuda")
        self._level_1_opacity_c1 = nn.Parameter(level_1_opacity_c1.requires_grad_(True))
        print(f"self._level_1_opacity_c1 inited {self._level_1_opacity_c1}")
        level_1_opacity_a2 = torch.zeros((level_1_total_parent_idx.shape[0], 1), device="cuda")
        self._level_1_opacity_a2 = nn.Parameter(level_1_opacity_a2.requires_grad_(True))
        print(f"self._level_1_opacity_a2 inited {self._level_1_opacity_a2}")
        level_1_opacity_c2 = torch.zeros((level_1_total_parent_idx.shape[0], 1), device="cuda")
        self._level_1_opacity_c2 = nn.Parameter(level_1_opacity_c2.requires_grad_(True))
        print(f"self._level_1_opacity_c2 inited {self._level_1_opacity_c2}")
        level_1_opacity_a3 = torch.zeros((level_1_total_parent_idx.shape[0], 1), device="cuda")
        self._level_1_opacity_a3 = nn.Parameter(level_1_opacity_a3.requires_grad_(True))
        print(f"self._level_1_opacity_a3 inited {self._level_1_opacity_a3}")
        level_1_opacity_c3 = torch.zeros((level_1_total_parent_idx.shape[0], 1), device="cuda")
        self._level_1_opacity_c3 = nn.Parameter(level_1_opacity_c3.requires_grad_(True))
        print(f"self._level_1_opacity_c3 inited {self._level_1_opacity_c3}")

        self._level_1_delta_rig_sur_radius = nn.Parameter(level_1_total_radius.requires_grad_(True))
        print(f"self._level_1_delta_rig_sur_radius inited {self._level_1_delta_rig_sur_radius}")

        self._level_1_delta_rig_sur_azimuth = nn.Parameter(level_1_total_azimuth.requires_grad_(True))
        print(f"self._level_1_delta_rig_sur_azimuth inited {self._level_1_delta_rig_sur_azimuth}")

        self._level_1_delta_rig_sur_polar = nn.Parameter(level_1_total_polar.requires_grad_(True))
        print(f"self._level_1_delta_rig_sur_polar inited {self._level_1_delta_rig_sur_polar}")
    ...
    def cache_level_1_gradient(self):
        ...
        self._level_1_opacity_a1 += self._level_1_opacity_a1.grad.clone()
        self._level_1_opacity_c1 += self._level_1_opacity_c1.grad.clone()
        self._level_1_opacity_a2 += self._level_1_opacity_a2.grad.clone()
        self._level_1_opacity_c2 += self._level_1_opacity_c2.grad.clone()
        self._level_1_opacity_a3 += self._level_1_opacity_a3.grad.clone()
        self._level_1_opacity_c3 += self._level_1_opacity_c3.grad.clone()
        self._level_1_delta_rig_sur_radius_grad += self._level_1_delta_rig_sur_radius.grad.clone()
        self._level_1_delta_rig_sur_azimuth_grad += self._level_1_delta_rig_sur_azimuth.grad.clone()
        self._level_1_delta_rig_sur_polar_grad += self._level_1_delta_rig_sur_polar.grad.clone()
    ...
    def zero_level_1_gradient_cache(self):
        ...
        self._level_1_opacity_a1 = torch.zeros_like(self._level_1_opacity_a1, requires_grad=False)
        self._level_1_opacity_c1 = torch.zeros_like(self._level_1_opacity_c1, requires_grad=False)
        self._level_1_opacity_a2 = torch.zeros_like(self._level_1_opacity_a2, requires_grad=False)
        self._level_1_opacity_c2 = torch.zeros_like(self._level_1_opacity_c2, requires_grad=False)
        self._level_1_opacity_a3 = torch.zeros_like(self._level_1_opacity_a3, requires_grad=False)
        self._level_1_opacity_c3 = torch.zeros_like(self._level_1_opacity_c3, requires_grad=False)
        self._level_1_delta_rig_sur_radius_grad = torch.zeros_like(
            self._level_1_delta_rig_sur_radius, requires_grad=False
        )
        self._level_1_delta_rig_sur_azimuth_grad = torch.zeros_like(
            self._level_1_delta_rig_sur_azimuth, requires_grad=False
        )
        self._level_1_delta_rig_sur_polar_grad = torch.zeros_like(
            self._level_1_delta_rig_sur_polar, requires_grad=False
        )
    ...
    def set_level_1_batch_gradient(self, batch_size):
        ...
        self._level_1_opacity_a1.grad = self._level_1_opacity_a1 * ratio
        self._level_1_opacity_c1.grad = self._level_1_opacity_c1 * ratio
        self._level_1_opacity_a2.grad = self._level_1_opacity_a2 * ratio
        self._level_1_opacity_c2.grad = self._level_1_opacity_c2 * ratio
        self._level_1_opacity_a3.grad = self._level_1_opacity_a3 * ratio
        self._level_1_opacity_c3.grad = self._level_1_opacity_c3 * ratio
        self._level_1_delta_rig_sur_radius.grad = self._level_1_delta_rig_sur_radius_grad * ratio
        self._level_1_delta_rig_sur_azimuth.grad = self._level_1_delta_rig_sur_azimuth_grad * ratio
        self._level_1_delta_rig_sur_polar.grad = self._level_1_delta_rig_sur_polar_grad * ratio
    ...
    def level_1_training_setup(self, training_args):
        ...
        l = [
            ...
            #     "name": "motion",
            # },
            {"params": [self._level_1_opacity_a1], "lr": training_args.level_1_opacity_a1_lr, "name": "opacity_a1"},
            {"params": [self._level_1_opacity_c1], "lr": training_args.level_1_opacity_c1_lr, "name": "opacity_c1"},
            {"params": [self._level_1_opacity_a2], "lr": training_args.level_1_opacity_a2_lr, "name": "opacity_a2"},
            {"params": [self._level_1_opacity_c2], "lr": training_args.level_1_opacity_c2_lr, "name": "opacity_c2"},
            {"params": [self._level_1_opacity_a3], "lr": training_args.level_1_opacity_a3_lr, "name": "opacity_a3"},
            {"params": [self._level_1_opacity_c3], "lr": training_args.level_1_opacity_c3_lr, "name": "opacity_c3"},
            {
                "params": [self._level_1_delta_rig_sur_radius],
                "lr": training_args.level_1_delta_rig_sur_radius_lr,
                "name": "delta_rig_sur_radius",
            },
            {
                "params": [self._level_1_delta_rig_sur_azimuth],
                "lr": training_args.level_1_delta_rig_sur_azimuth_lr,
                "name": "delta_rig_sur_azimuth",
            },
            {
                "params": [self._level_1_delta_rig_sur_polar],
                "lr": training_args.level_1_delta_rig_sur_polar_lr,
                "name": "delta_rig_sur_polar",
            },
        ]
    ...
    def save_ply(self, path):
        ...
        delta_rig_sur_radius_dummy = np.zeros((xyz.shape[0], 1))
        delta_rig_sur_azimuth_dummy = np.zeros((xyz.shape[0], 1))
        delta_rig_sur_polar_dummy = np.zeros((xyz.shape[0], 1))
        opacity_a1_dummy = np.zeros((xyz.shape[0], 1))
        opacity_c1_dummy = np.zeros((xyz.shape[0], 1))
        opacity_a2_dummy = np.zeros((xyz.shape[0], 1))
        opacity_c2_dummy = np.zeros((xyz.shape[0], 1))
        opacity_a3_dummy = np.zeros((xyz.shape[0], 1))
        opacity_c3_dummy = np.zeros((xyz.shape[0], 1))
        ...
        if self._level_1_parent_idx.shape[0] > 0:
            ...
            level_1_delta_rig_sur_radius = self._level_1_delta_rig_sur_radius.detach().cpu().numpy()
            level_1_delta_rig_sur_azimuth = self._level_1_delta_rig_sur_azimuth.detach().cpu().numpy()
            level_1_delta_rig_sur_polar = self._level_1_delta_rig_sur_polar.detach().cpu().numpy()

            level_1_opacity_a1 = self._level_1_opacity_a1.detach().cpu().numpy()
            level_1_opacity_c1 = self._level_1_opacity_c1.detach().cpu().numpy()
            level_1_opacity_a2 = self._level_1_opacity_a2.detach().cpu().numpy()
            level_1_opacity_c2 = self._level_1_opacity_c2.detach().cpu().numpy()
            level_1_opacity_a3 = self._level_1_opacity_a3.detach().cpu().numpy()
            level_1_opacity_c3 = self._level_1_opacity_c3.detach().cpu().numpy()
            ...
            all_delta_rig_sur_radius = np.concatenate(
                (delta_rig_sur_radius_dummy, level_1_delta_rig_sur_radius), axis=0
            )
            all_delta_rig_sur_azimuth = np.concatenate(
                (delta_rig_sur_azimuth_dummy, level_1_delta_rig_sur_azimuth), axis=0
            )
            all_delta_rig_sur_polar = np.concatenate((delta_rig_sur_polar_dummy, level_1_delta_rig_sur_polar), axis=0)
            all_opacity_a1 = np.concatenate((opacity_a1_dummy, level_1_opacity_a1), axis=0)
            all_opacity_c1 = np.concatenate((opacity_c1_dummy, level_1_opacity_c1), axis=0)
            all_opacity_a2 = np.concatenate((opacity_a2_dummy, level_1_opacity_a2), axis=0)
            all_opacity_c2 = np.concatenate((opacity_c2_dummy, level_1_opacity_c2), axis=0)
            all_opacity_a3 = np.concatenate((opacity_a3_dummy, level_1_opacity_a3), axis=0)
            all_opacity_c3 = np.concatenate((opacity_c3_dummy, level_1_opacity_c3), axis=0)
            ...
            all_delta_rig_sur_radius = delta_rig_sur_radius_dummy
            all_delta_rig_sur_azimuth = delta_rig_sur_azimuth_dummy
            all_delta_rig_sur_polar = delta_rig_sur_polar_dummy

            all_opacity_a1 = opacity_a1_dummy
            all_opacity_c1 = opacity_c1_dummy
            all_opacity_a2 = opacity_a2_dummy
            all_opacity_c2 = opacity_c2_dummy
            all_opacity_a3 = opacity_a3_dummy
            all_opacity_c3 = opacity_c3_dummy
    ...
    def load_ply(self, path):
        ...
        delta_rig_sur_radius = np.asarray(ply_data.elements[0]["delta_rig_sur_radius"])[..., np.newaxis]
        delta_rig_sur_azimuth = np.asarray(ply_data.elements[0]["delta_rig_sur_azimuth"])[..., np.newaxis]
        delta_rig_sur_polar = np.asarray(ply_data.elements[0]["delta_rig_sur_polar"])[..., np.newaxis]
        ...
        self._delta_rig_sur_radius = torch.tensor(delta_rig_sur_radius[level_0_idx], dtype=torch.float, device="cuda")
        self._delta_rig_sur_azimuth = torch.tensor(
            delta_rig_sur_azimuth[level_0_idx], dtype=torch.float, device="cuda"
        )
        self._delta_rig_sur_polar = torch.tensor(delta_rig_sur_polar[level_0_idx], dtype=torch.float, device="cuda")
        ...
        if np.sum(level_1_idx) > 0:
            ...
            self._level_1_delta_rig_sur_radius = nn.Parameter(
                torch.tensor(delta_rig_sur_radius[level_1_idx], dtype=torch.float, device="cuda").requires_grad_(True)
            )

            self._level_1_delta_rig_sur_azimuth = nn.Parameter(
                torch.tensor(delta_rig_sur_azimuth[level_1_idx], dtype=torch.float, device="cuda").requires_grad_(True)
            )
            self._level_1_delta_rig_sur_polar = nn.Parameter(
                torch.tensor(delta_rig_sur_polar[level_1_idx], dtype=torch.float, device="cuda").requires_grad_(True)
            )
    ...
    def prune_points_level_1(self, mask):
        ...
        self._level_1_delta_rig_sur_azimuth = optimizable_tensors["delta_rig_sur_azimuth"]
        self._level_1_delta_rig_sur_polar = optimizable_tensors["delta_rig_sur_polar"]
        self._level_1_opacity_a1 = optimizable_tensors["opacity_a1"]
        self._level_1_opacity_c1 = optimizable_tensors["opacity_c1"]
        self._level_1_opacity_a2 = optimizable_tensors["opacity_a2"]
        self._level_1_opacity_c2 = optimizable_tensors["opacity_c2"]
        self._level_1_opacity_a3 = optimizable_tensors["opacity_a3"]
        self._level_1_opacity_c3 = optimizable_tensors["opacity_c3"]
    ...
    def densification_postfix_level_1(
        ...
        new_delta_rig_sur_azimuth,
        new_delta_rig_sur_polar,
        new_opacity_a1,
        new_opacity_c1,
        new_opacity_a2,
        new_opacity_c2,
        ...
        d = {
            ...
            "delta_rig_sur_azimuth": new_delta_rig_sur_azimuth,
            "delta_rig_sur_polar": new_delta_rig_sur_polar,
            "opacity_a1": new_opacity_a1,
            "opacity_c1": new_opacity_c1,
            "opacity_a2": new_opacity_a2,
            "opacity_c2": new_opacity_c2,
            "opacity_a3": new_opacity_a3,
            "opacity_c3": new_opacity_c3,
        }
    ...
    def densify_and_split_level_1(self, grads, grad_threshold, scene_extent, max_timestamp=1.0, N=2):
        ...
        new_delta_rig_sur_azimuth = self._level_1_delta_rig_sur_azimuth[selected_pts_mask].repeat(N, 1)
        new_delta_rig_sur_polar = self._level_1_delta_rig_sur_polar[selected_pts_mask].repeat(N, 1)
        new_opacity_a1 = self._level_1_opacity_a1[selected_pts_mask].repeat(N, 1)
        new_opacity_c1 = self._level_1_opacity_c1[selected_pts_mask].repeat(N, 1)
        new_opacity_a2 = self._level_1_opacity_a2[selected_pts_mask].repeat(N, 1)
        new_opacity_c2 = self._level_1_opacity_c2[selected_pts_mask].repeat(N, 1)
        new_opacity_a3 = self._level_1_opacity_a3[selected_pts_mask].repeat(N, 1)
        new_opacity_c3 = self._level_1_opacity_c3[selected_pts_mask].repeat(N, 1)
    ...
    def densify_and_clone_level_1(self, grads, grad_threshold, scene_extent, max_timestamp=1.0):
        ...
        new_delta_rig_sur_azimuth = self._level_1_delta_rig_sur_azimuth[selected_pts_mask]
        new_delta_rig_sur_polar = self._level_1_delta_rig_sur_polar[selected_pts_mask]
        new_opacity_a1 = self._level_1_opacity_a1[selected_pts_mask]
        new_opacity_c1 = self._level_1_opacity_c1[selected_pts_mask]
        new_opacity_a2 = self._level_1_opacity_a2[selected_pts_mask]
        new_opacity_c2 = self._level_1_opacity_c2[selected_pts_mask]
        new_opacity_a3 = self._level_1_opacity_a3[selected_pts_mask]
        new_opacity_c3 = self._level_1_opacity_c3[selected_pts_mask]
        self.densification_postfix_level_1(
            new_parent_idx,
            new_features_dc,
            new_opacities,
