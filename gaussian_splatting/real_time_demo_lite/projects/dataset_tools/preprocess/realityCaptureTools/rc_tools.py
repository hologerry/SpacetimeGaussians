...
def preprocess_for_rc(path, video_name="default", do_validation_split=True, valid_skip="10"):
    ...
    video_path = os.path.abspath(os.path.join(rawpath, "videos"))
    do_test = False
    ...
    if not os.path.exists(imagespath):
        ...
        elif not os.path.exists(os.path.join(path, "videos")) and not os.path.exists(video_path):
            print("ERROR: No images nor video, exiting. Images should be in $path/raw/images")
            exit(-1)
        # videos are optional
        if os.path.exists(os.path.join(path, "videos")):
            shutil.move(os.path.join(path, "videos"), video_path)
        ...
        if os.path.exists(video_path):
            print("Found video {}".format(video_path))
...
def convert_sibr_mesh(path):
    ...
    out_mesh_path = os.path.join(
        os.path.join(os.path.join(os.path.join(path, "sibr"), "colmap"), "stereo"), "meshed-delaunay.ply"
    )
...
def rc_to_colmap(rc_path, out_path, create_colmap=False, target_width=-1):
    ...
    with open(fname, "w") as outfile:
        ...
        for im in input_bundle.list_of_input_images:
            ...
            outfile.write(
                "{} PINHOLE {} {} {} {} {} {}\n".format(
                    camera_id, int(width), int(height), focal_length, focal_length, width / 2.0, height / 2.0
                )
            )
        ...
        for cam in input_bundle.list_of_cameras:
            ...
            for p in cam.list_of_feature_points:
                for v in p.view_list:
                    if v[0] == camera_id - 1:
                        outfile.write(
                            str(scale * (2.0 * v[2] + w)) + " " + str(scale * (2.0 * v[3] + h)) + " -1"
...
def car_data_process(path):
    ...
    for dirname in dirlist:
        ...
        for filename in os.listdir(campath):
            shutil.copyfile(
                os.path.join(campath, filename), os.path.join(imagespath, "{:06d}".format(global_im_counter) + ".jpg")
            )
            global_im_counter += 1
