...
if target_res is not None:
    ...
    crop_from_center_args.extend(
        ["--scaleDownFactor", str(scale_factor), "--targetResolution", str(target_res[0]), str(target_res[1])]
    )
...
if os.path.exists(os.path.join(path_dest, pmvs_model_dir, "pmvs_recon.ply")):
    shutil.copy(
        os.path.join(path_dest, pmvs_model_dir, "pmvs_recon.ply"), os.path.join(path_dest, pmvs_model_dir, "recon.ply")
    )

if os.path.exists(os.path.join(path_dest, pmvs_model_dir, "mesh.ply")):
    shutil.copy(
        os.path.join(path_dest, pmvs_model_dir, "mesh.ply"), os.path.join(path_dest, pmvs_model_dir, "recon.ply")
    )
