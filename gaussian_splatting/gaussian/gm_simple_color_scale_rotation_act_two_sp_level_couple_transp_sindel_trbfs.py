...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int, rgb_function="rgbv1"):
        ...
        self._delta_sin_a = torch.empty(0)  # dummy
        self._delta_sin_omega = torch.empty(0)  # dummy
        self._delta_sin_phi = torch.empty(0)  # dummy
        ...
        self._level_1_delta_sin_a = torch.empty(0)
        self._level_1_delta_sin_omega = torch.empty(0)
        self._level_1_delta_sin_phi = torch.empty(0)
    ...
    @property
    def get_level_1_delta_sin_a(self):
        return self._level_1_delta_sin_a

    @property
    def get_level_1_delta_sin_omega(self):
        return self._level_1_delta_sin_omega

    @property
    def get_level_1_delta_sin_phi(self):
        return self._level_1_delta_sin_phi
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        self._delta_sin_a = torch.zeros((fused_point_cloud.shape[0], 9), device="cuda")
        self._delta_sin_omega = torch.zeros((fused_point_cloud.shape[0], 9), device="cuda")
        self._delta_sin_phi = torch.zeros((fused_point_cloud.shape[0], 9), device="cuda")
        print(f"self._delta_sin_a inited {self._delta_sin_a}")
    ...
    def create_another_level(
        ...
        new_pts_init_delta_sin_a=1.0,
        new_pts_init_delta_sin_omega=1.0,
        new_pts_init_delta_sin_phi=0.0,
        start_time=0,
        duration=120,
        time_step=1,
        **kwargs,
        ...
        # by default, we use three sin functions to represent the delta xyz
        level_1_delta_sin_a = (
            torch.zeros((level_1_total_parent_idx.shape[0], 9), device="cuda") + new_pts_init_delta_sin_a
        )
        self._level_1_delta_sin_a = nn.Parameter(level_1_delta_sin_a.requires_grad_(True))
        level_1_delta_sin_omega = (
            torch.zeros((level_1_total_parent_idx.shape[0], 9), device="cuda") + new_pts_init_delta_sin_omega
        )
        self._level_1_delta_sin_omega = nn.Parameter(level_1_delta_sin_omega.requires_grad_(True))
        level_1_delta_sin_phi = (
            torch.zeros((level_1_total_parent_idx.shape[0], 9), device="cuda") + new_pts_init_delta_sin_phi
        )
        self._level_1_delta_sin_phi = nn.Parameter(level_1_delta_sin_phi.requires_grad_(True))
        print(f"self._level_1_delta_sin_a inited {self._level_1_delta_sin_a}")
        print(f"self._level_1_delta_sin_omega inited {self._level_1_delta_sin_omega}")
        print(f"self._level_1_delta_sin_phi inited {self._level_1_delta_sin_phi}")
    ...
    def cache_level_1_gradient(self):
        ...
        self._level_1_delta_sin_a_grad += self._level_1_delta_sin_a.grad.clone()
        self._level_1_delta_sin_omega_grad += self._level_1_delta_sin_omega.grad.clone()
        self._level_1_delta_sin_phi_grad += self._level_1_delta_sin_phi.grad.clone()
    ...
    def zero_level_1_gradient_cache(self):
        ...
        self._level_1_delta_sin_a_grad = torch.zeros_like(self._level_1_delta_sin_a, requires_grad=False)
        self._level_1_delta_sin_omega_grad = torch.zeros_like(self._level_1_delta_sin_omega, requires_grad=False)
        self._level_1_delta_sin_phi_grad = torch.zeros_like(self._level_1_delta_sin_phi, requires_grad=False)
    ...
    def set_level_1_batch_gradient(self, batch_size):
        ...
        self._level_1_delta_sin_a.grad = self._level_1_delta_sin_a_grad * ratio
        self._level_1_delta_sin_omega.grad = self._level_1_delta_sin_omega_grad * ratio
        self._level_1_delta_sin_phi.grad = self._level_1_delta_sin_phi_grad * ratio
    ...
    def level_1_training_setup(self, training_args):
        ...
        l = [
            ...
            #     "name": "motion",
            # },
            {"params": [self._level_1_delta_sin_a], "lr": training_args.level_1_delta_sin_a_lr, "name": "delta_sin_a"},
            {
                "params": [self._level_1_delta_sin_omega],
                "lr": training_args.level_1_delta_sin_omega_lr,
                "name": "delta_sin_omega",
            },
            {
                "params": [self._level_1_delta_sin_phi],
                "lr": training_args.level_1_delta_sin_phi_lr,
                "name": "delta_sin_phi",
            },
        ]
    ...
    def construct_list_of_attributes(self):
        ...
        for i in range(self._delta_sin_a.shape[1]):
            l.append(f"delta_sin_a_{i}")
        for i in range(self._delta_sin_omega.shape[1]):
            l.append(f"delta_sin_omega_{i}")
        for i in range(self._delta_sin_phi.shape[1]):
            l.append(f"delta_sin_phi_{i}")
    ...
    def save_ply(self, path):
        ...
        delta_sin_a_dummy = np.zeros((xyz.shape[0], 9))
        delta_sin_omega_dummy = np.zeros((xyz.shape[0], 9))
        delta_sin_phi_dummy = np.zeros((xyz.shape[0], 9))
        ...
        if self._level_1_parent_idx.shape[0] > 0:
            ...
            level_1_sin_a = self._level_1_delta_sin_a.detach().cpu().numpy()
            level_1_sin_omega = self._level_1_delta_sin_omega.detach().cpu().numpy()
            level_1_sin_phi = self._level_1_delta_sin_phi.detach().cpu().numpy()
            ...
            all_delta_sin_a = np.concatenate((delta_sin_a_dummy, level_1_sin_a), axis=0)
            all_delta_sin_omega = np.concatenate((delta_sin_omega_dummy, level_1_sin_omega), axis=0)
            all_delta_sin_phi = np.concatenate((delta_sin_phi_dummy, level_1_sin_phi), axis=0)
            ...
            all_delta_sin_a = delta_sin_a_dummy
            all_delta_sin_omega = delta_sin_omega_dummy
            all_delta_sin_phi = delta_sin_phi_dummy
    ...
    def load_ply(self, path):
        ...
        delta_sin_a_names = [p.name for p in ply_data.elements[0].properties if p.name.startswith("delta_sin_a")]
        delta_sin_a = np.zeros((xyz.shape[0], len(delta_sin_a_names)))
        for idx, attr_name in enumerate(delta_sin_a_names):
            delta_sin_a[:, idx] = np.asarray(ply_data.elements[0][attr_name])

        delta_sin_omega_names = [
            p.name for p in ply_data.elements[0].properties if p.name.startswith("delta_sin_omega")
        ]
        delta_sin_omega = np.zeros((xyz.shape[0], len(delta_sin_omega_names)))
        for idx, attr_name in enumerate(delta_sin_omega_names):
            delta_sin_omega[:, idx] = np.asarray(ply_data.elements[0][attr_name])

        delta_sin_phi_names = [p.name for p in ply_data.elements[0].properties if p.name.startswith("delta_sin_phi")]
        delta_sin_phi = np.zeros((xyz.shape[0], len(delta_sin_phi_names)))
        for idx, attr_name in enumerate(delta_sin_phi_names):
            delta_sin_phi[:, idx] = np.asarray(ply_data.elements[0][attr_name])
        ...
        self._delta_sin_a = torch.tensor(delta_sin_a[level_0_idx], dtype=torch.float, device="cuda")
        self._delta_sin_omega = torch.tensor(delta_sin_omega[level_0_idx], dtype=torch.float, device="cuda")
        self._delta_sin_phi = torch.tensor(delta_sin_phi[level_0_idx], dtype=torch.float, device="cuda")
        ...
        if np.sum(level_1_idx) > 0:
            ...
            self._level_1_delta_sin_a = nn.Parameter(
                torch.tensor(delta_sin_a[level_1_idx], dtype=torch.float, device="cuda").requires_grad_(True)
            )
            self._level_1_delta_sin_omega = nn.Parameter(
                torch.tensor(delta_sin_omega[level_1_idx], dtype=torch.float, device="cuda").requires_grad_(True)
            )
            self._level_1_delta_sin_phi = nn.Parameter(
                torch.tensor(delta_sin_phi[level_1_idx], dtype=torch.float, device="cuda").requires_grad_(True)
            )
    ...
    def prune_points_level_1(self, mask):
        ...
        self._level_1_delta_sin_a = optimizable_tensors["delta_sin_a"]
        self._level_1_delta_sin_omega = optimizable_tensors["delta_sin_omega"]
        self._level_1_delta_sin_phi = optimizable_tensors["delta_sin_phi"]
    ...
    def densification_postfix_level_1(
        ...
        d = {
            ...
            # "motion": new_motion,
            "omega": new_omega,
            "delta_sin_a": new_delta_sin_a,
            "delta_sin_omega": new_delta_sin_omega,
            "delta_sin_phi": new_delta_sin_phi,
        }
    ...
    def densify_and_split_level_1(self, grads, grad_threshold, scene_extent, max_timestamp=1.0, N=2):
        ...
        new_delta_sin_a = self._level_1_delta_sin_a[selected_pts_mask].repeat(N, 1)
        new_delta_sin_omega = self._level_1_delta_sin_omega[selected_pts_mask].repeat(N, 1)
        new_delta_sin_phi = self._level_1_delta_sin_phi[selected_pts_mask].repeat(N, 1)
    ...
    def densify_and_clone_level_1(self, grads, grad_threshold, scene_extent, max_timestamp=1.0):
        ...
        new_delta_sin_a = self._level_1_delta_sin_a[selected_pts_mask]
        new_delta_sin_omega = self._level_1_delta_sin_omega[selected_pts_mask]
        new_delta_sin_phi = self._level_1_delta_sin_phi[selected_pts_mask]
