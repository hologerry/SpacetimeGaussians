...
def checkColmapConsistent(path_dir):
    colmapDir = Path(path_dir + "/colmap")

    if not os.path.isdir(colmapDir):
        return False

    # check for mesh
    colmapmesh = Path(path_dir + "/colmap/stereo/meshed-delaunay.ply")
...
def main():
    ...
    p_exit = subprocess.call(
        ["python", "ibr_preprocess_rc_to_sibr.py", "-i", args["path"] + "/sfm_mvs_cm", "-o", args["path"] + "/sibr_cm"]
    )
    if p_exit != 0:
