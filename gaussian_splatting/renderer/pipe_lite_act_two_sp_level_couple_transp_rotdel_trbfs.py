...
def train_lite_act_two_sp_level_couple_transp_rotdel_trbfs(
    viewpoint_camera,
    gm: GaussianModel,
    pipe,
    ...
    **kwargs,
    ...
    if level == 0:
        ...
        level_1_delta_rot_radius = gm.get_level_1_delta_rot_radius
        level_1_delta_rot_angle_vel = gm.get_level_1_delta_rot_angle_vel
        ...
        if rotdel_type == "xz":
            level_1_delta_means3D[:, 0:1] = level_1_delta_rot_radius * torch.cos(
                level_1_delta_rot_angle_vel * viewpoint_camera.timestamp * 2.0 * math.pi
            )
            level_1_delta_means3D[:, 1:2] = 0.0
            level_1_delta_means3D[:, 2:3] = level_1_delta_rot_radius * torch.sin(
                level_1_delta_rot_angle_vel * viewpoint_camera.timestamp * 2.0 * math.pi
            )
        elif rotdel_type == "xy":
            level_1_delta_means3D[:, 0:1] = level_1_delta_rot_radius * torch.cos(
            ...
            level_1_delta_means3D[:, 1:2] = level_1_delta_rot_radius * torch.sin(
                level_1_delta_rot_angle_vel * viewpoint_camera.timestamp * 2.0 * math.pi
            )
            level_1_delta_means3D[:, 2:3] = 0.0

        elif rotdel_type == "yz":
            level_1_delta_means3D[:, 0:1] = 0.0
            level_1_delta_means3D[:, 1:2] = level_1_delta_rot_radius * torch.cos(
                level_1_delta_rot_angle_vel * viewpoint_camera.timestamp * 2.0 * math.pi
            )
            level_1_delta_means3D[:, 2:3] = level_1_delta_rot_radius * torch.sin(
                level_1_delta_rot_angle_vel * viewpoint_camera.timestamp * 2.0 * math.pi
            ...
            level_1_delta_means3D[:, 2:3] = level_1_delta_rot_radius * torch.cos(
                level_1_delta_rot_angle_vel * viewpoint_camera.timestamp * 2.0 * math.pi
            )
        else:
            raise ValueError(f"Invalid rotdel_type: {rotdel_type}")
...
def test_lite_act_two_sp_level_couple_transp_rotdel_trbfs_vis(
    viewpoint_camera,
    gm: GaussianModel,
    pipe,
    bg_color: torch.Tensor,
    ...
    **kwargs,
    ...
    if level == 0:
        ...
        if rotdel_type == "xz":
            ...
            level_1_delta_velocities3D[:, 0:1] = (
                level_1_delta_rot_angle_vel
                * torch.cos(level_1_delta_rot_angle_vel * viewpoint_camera.timestamp * 2.0 * math.pi)
                * 2.0
                * math.pi
                * level_1_delta_rot_angle_vel
            )
            level_1_delta_velocities3D[:, 1:2] = 0.0
            level_1_delta_velocities3D[:, 2:3] = (
                -level_1_delta_rot_angle_vel
                * torch.sin(level_1_delta_rot_angle_vel * viewpoint_camera.timestamp * 2.0 * math.pi)
                * 2.0
                * math.pi
                * level_1_delta_rot_angle_vel
            )
            ...
            level_1_delta_velocities3D[:, 1:2] = (
                level_1_delta_rot_angle_vel
                * torch.sin(level_1_delta_rot_angle_vel * viewpoint_camera.timestamp * 2.0 * math.pi)
                * 2.0
                * math.pi
            ...
            level_1_delta_velocities3D[:, 2:3] = 0.0
            ...
            level_1_delta_velocities3D = torch.zeros_like(level_1_parent_velocity3D)
            level_1_delta_velocities3D[:, 0:1] = 0.0
            level_1_delta_velocities3D[:, 1:2] = (
                level_1_delta_rot_angle_vel
                * torch.cos(level_1_delta_rot_angle_vel * viewpoint_camera.timestamp * 2.0 * math.pi)
                * 2.0
