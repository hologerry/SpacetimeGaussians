...
def train_lite_act_two_sp_level_couple_transp_trotdel_trbfs(
    viewpoint_camera,
    gm: GaussianModel,
    pipe,
    ...
    **kwargs,
    ...
    if level == 0:
        ...
        level_1_delta_trot_radius = gm.get_level_1_delta_trot_radius(viewpoint_camera.timestamp)
        level_1_delta_trot_angle_vel = gm.get_level_1_delta_trot_angle_vel(viewpoint_camera.timestamp)

        level_1_parent_idx = level_1_parent_idx.squeeze(1)

        level_1_parent_means3D = level_0_cur_means3D[level_1_parent_idx]

        level_1_delta_means3D = level_1_delta_trot_radius * torch.sin(
            level_1_delta_trot_angle_vel * viewpoint_camera.timestamp * 2.0 * math.pi
        )
...
def test_lite_act_two_sp_level_couple_transp_trotdel_trbfs_vis(
    viewpoint_camera,
    gm: GaussianModel,
    pipe,
    bg_color: torch.Tensor,
    ...
    **kwargs,
    ...
    level_0_delta_trot_center_dummy = torch.zeros((n_level_0_points, 1), dtype=level_0_means3D.dtype, device="cuda")
    level_0_delta_trot_radius_dummy = torch.zeros((n_level_0_points, 3), dtype=level_0_means3D.dtype, device="cuda")
    level_0_delta_trot_alpha_dummy = torch.zeros((n_level_0_points, 3), dtype=level_0_means3D.dtype, device="cuda")
    level_0_delta_trot_angle_vel_dummy = torch.zeros((n_level_0_points, 3), dtype=level_0_means3D.dtype, device="cuda")
    level_0_delta_trot_beta_dummy = torch.zeros((n_level_0_points, 3), dtype=level_0_means3D.dtype, device="cuda")
    ...
    if level == 0:
        ...
        delta_trot_center = level_0_delta_trot_center_dummy
        delta_trot_radius = level_0_delta_trot_radius_dummy
        delta_trot_alpha = level_0_delta_trot_alpha_dummy
        delta_trot_angle_vel = level_0_delta_trot_angle_vel_dummy
        delta_trot_beta = level_0_delta_trot_beta_dummy
        ...
        level_1_delta_trot_center = gm._level_1_delta_trot_center
        level_1_delta_trot_radius = gm.get_level_1_delta_trot_radius(viewpoint_camera.timestamp)
        level_1_delta_trot_alpha = gm._level_1_delta_trot_alpha
        level_1_delta_trot_angle_vel = gm.get_level_1_delta_trot_angle_vel(viewpoint_camera.timestamp)
        level_1_delta_trot_beta = gm._level_1_delta_trot_beta
        ...
        # (r + alpha(t-t0)) * sin(2π * t * (v + beta(t-t0)))
        level_1_delta_means3D = level_1_delta_trot_radius * torch.sin(
            level_1_delta_trot_angle_vel * viewpoint_camera.timestamp * 2.0 * math.pi
        )

        # a * sin(2 * π * x * (b * x + v)) + (a * x + r) * (2 * π * (b * x + v) + 2 * π * b * x) * cos(2 * π * x * (b * x + v))
        level_1_delta_velocities3D = gm._level_1_delta_trot_alpha * torch.sin(
            2
            * math.pi
            * viewpoint_camera.timestamp
            * (gm._level_1_delta_trot_beta * viewpoint_camera.timestamp + gm._level_1_delta_trot_center)
        ) + (gm._level_1_delta_trot_alpha * viewpoint_camera.timestamp + gm._level_1_delta_trot_radius) * (
            2 * math.pi * (gm._level_1_delta_trot_beta * viewpoint_camera.timestamp + gm._level_1_delta_trot_center)
            + 2 * math.pi * gm._level_1_delta_trot_beta * viewpoint_camera.timestamp
        ) * torch.cos(
            2
            * math.pi
            * viewpoint_camera.timestamp
        ...
        if transp_level_0:
            ...
            delta_trot_center = level_1_delta_trot_center
            delta_trot_radius = level_1_delta_trot_radius
            delta_trot_alpha = level_1_delta_trot_alpha
            delta_trot_angle_vel = level_1_delta_trot_angle_vel
            delta_trot_beta = level_1_delta_trot_beta
            ...
            delta_trot_center = torch.cat((level_0_delta_trot_center_dummy, level_1_delta_trot_center), dim=0)
            delta_trot_radius = torch.cat((level_0_delta_trot_radius_dummy, level_1_delta_trot_radius), dim=0)
            delta_trot_alpha = torch.cat((level_0_delta_trot_alpha_dummy, level_1_delta_trot_alpha), dim=0)
            delta_trot_angle_vel = torch.cat((level_0_delta_trot_angle_vel_dummy, level_1_delta_trot_angle_vel), dim=0)
            delta_trot_beta = torch.cat((level_0_delta_trot_beta_dummy, level_1_delta_trot_beta), dim=0)
    ...
    # since the level_0 level_1 xyz logic is different, we move the xyz calculation out cuda kernel
    ...
    return {
        ...
        "delta_trot_center": delta_trot_center,
        "delta_trot_radius": delta_trot_radius,
        "delta_trot_alpha": delta_trot_alpha,
        "delta_trot_angle_vel": delta_trot_angle_vel,
        "delta_trot_beta": delta_trot_beta,
    }
