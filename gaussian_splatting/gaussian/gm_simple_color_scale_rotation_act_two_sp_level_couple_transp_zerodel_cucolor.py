...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int, rgb_function="rgbv1"):
        ...
        self._level_1_color_a1 = torch.empty(0)
        self._level_1_color_c1 = torch.empty(0)
        self._level_1_color_a2 = torch.empty(0)
        self._level_1_color_c2 = torch.empty(0)
        self._level_1_color_a3 = torch.empty(0)
        self._level_1_color_c3 = torch.empty(0)
    ...
    @property
    def get_level_1_color_a1(self):
        return self._level_1_color_a1

    @property
    def get_level_1_color_c1(self):
        return self._level_1_color_c1

    @property
    def get_level_1_color_a2(self):
        return self._level_1_color_a2

    @property
    def get_level_1_color_c2(self):
        return self._level_1_color_c2

    @property
    def get_level_1_color_a3(self):
        return self._level_1_color_a3

    @property
    def get_level_1_color_c3(self):
        return self._level_1_color_c3
    ...
    def create_another_level(
        ...
        **kwargs,
        ...
        level_1_color_a1 = torch.zeros((level_1_total_parent_idx.shape[0], self._features_dc.shape[1]), device="cuda")
        self._level_1_color_a1 = nn.Parameter(level_1_color_a1.requires_grad_(True))
        print(f"self._level_1_color_a1 inited {self._level_1_color_a1}")

        level_1_color_c1 = torch.zeros((level_1_total_parent_idx.shape[0], self._features_dc.shape[1]), device="cuda")
        self._level_1_color_c1 = nn.Parameter(level_1_color_c1.requires_grad_(True))

        level_1_color_a2 = torch.zeros((level_1_total_parent_idx.shape[0], self._features_dc.shape[1]), device="cuda")
        self._level_1_color_a2 = nn.Parameter(level_1_color_a2.requires_grad_(True))
        print(f"self._level_1_color_a2 inited {self._level_1_color_a2}")

        level_1_color_c2 = torch.zeros((level_1_total_parent_idx.shape[0], self._features_dc.shape[1]), device="cuda")
        self._level_1_color_c2 = nn.Parameter(level_1_color_c2.requires_grad_(True))

        level_1_color_a3 = torch.zeros((level_1_total_parent_idx.shape[0], self._features_dc.shape[1]), device="cuda")
        self._level_1_color_a3 = nn.Parameter(level_1_color_a3.requires_grad_(True))
        print(f"self._level_1_color_a3 inited {self._level_1_color_a3}")

        level_1_color_c3 = torch.zeros((level_1_total_parent_idx.shape[0], self._features_dc.shape[1]), device="cuda")
        self._level_1_color_c3 = nn.Parameter(level_1_color_c3.requires_grad_(True))
    ...
    def cache_level_1_gradient(self):
        ...
        self._level_1_color_a1_grad += self._level_1_color_a1.grad.clone()
        self._level_1_color_c1_grad += self._level_1_color_c1.grad.clone()
        self._level_1_color_a2_grad += self._level_1_color_a2.grad.clone()
        self._level_1_color_c2_grad += self._level_1_color_c2.grad.clone()
        self._level_1_color_a3_grad += self._level_1_color_a3.grad.clone()
        self._level_1_color_c3_grad += self._level_1_color_c3.grad.clone()
    ...
    def zero_level_1_gradient_cache(self):
        ...
        self._level_1_color_a1_grad = torch.zeros_like(self._level_1_color_a1, requires_grad=False)
        self._level_1_color_c1_grad = torch.zeros_like(self._level_1_color_c1, requires_grad=False)
        self._level_1_color_a2_grad = torch.zeros_like(self._level_1_color_a2, requires_grad=False)
        self._level_1_color_c2_grad = torch.zeros_like(self._level_1_color_c2, requires_grad=False)
        self._level_1_color_a3_grad = torch.zeros_like(self._level_1_color_a3, requires_grad=False)
        self._level_1_color_c3_grad = torch.zeros_like(self._level_1_color_c3, requires_grad=False)
    ...
    def set_level_1_batch_gradient(self, batch_size):
        ...
        self._level_1_color_a1.grad = self._level_1_color_a1_grad * ratio
        self._level_1_color_c1.grad = self._level_1_color_c1_grad * ratio
        self._level_1_color_a2.grad = self._level_1_color_a2_grad * ratio
        self._level_1_color_c2.grad = self._level_1_color_c2_grad * ratio
        self._level_1_color_a3.grad = self._level_1_color_a3_grad * ratio
        self._level_1_color_c3.grad = self._level_1_color_c3_grad * ratio
    ...
    def level_1_training_setup(self, training_args):
        ...
        l = [
            ...
            #     "name": "motion",
            # },
            {"params": [self._level_1_color_a1], "lr": training_args.level_1_color_a1_lr, "name": "color_a1"},
            {"params": [self._level_1_color_c1], "lr": training_args.level_1_color_c1_lr, "name": "color_c1"},
            {"params": [self._level_1_color_a2], "lr": training_args.level_1_color_a2_lr, "name": "color_a2"},
            {"params": [self._level_1_color_c2], "lr": training_args.level_1_color_c2_lr, "name": "color_c2"},
            {"params": [self._level_1_color_a3], "lr": training_args.level_1_color_a3_lr, "name": "color_a3"},
            {"params": [self._level_1_color_c3], "lr": training_args.level_1_color_c3_lr, "name": "color_c3"},
        ]
    ...
    def construct_list_of_attributes(self):
        ...
        for i in range(self._features_dc.shape[1]):
            l.append(f"color_a1_{i}")
        for i in range(self._features_dc.shape[1]):
            l.append(f"color_c1_{i}")
        for i in range(self._features_dc.shape[1]):
            l.append(f"color_a2_{i}")
        for i in range(self._features_dc.shape[1]):
            l.append(f"color_c2_{i}")
        for i in range(self._features_dc.shape[1]):
            l.append(f"color_a3_{i}")
        for i in range(self._features_dc.shape[1]):
            l.append(f"color_c3_{i}")
    ...
    def save_ply(self, path):
        ...
        color_a1_dummy = np.zeros_like(f_dc)
        color_c1_dummy = np.zeros_like(f_dc)
        color_a2_dummy = np.zeros_like(f_dc)
        color_c2_dummy = np.zeros_like(f_dc)
        color_a3_dummy = np.zeros_like(f_dc)
        color_c3_dummy = np.zeros_like(f_dc)
        ...
        if self._level_1_parent_idx.shape[0] > 0:
            ...
            level_1_color_a1 = self._level_1_color_a1.detach().cpu().numpy()
            level_1_color_c1 = self._level_1_color_c1.detach().cpu().numpy()
            level_1_color_a2 = self._level_1_color_a2.detach().cpu().numpy()
            level_1_color_c2 = self._level_1_color_c2.detach().cpu().numpy()
            level_1_color_a3 = self._level_1_color_a3.detach().cpu().numpy()
            level_1_color_c3 = self._level_1_color_c3.detach().cpu().numpy()
            ...
            all_color_a1 = np.concatenate((color_a1_dummy, level_1_color_a1), axis=0)
            all_color_c1 = np.concatenate((color_c1_dummy, level_1_color_c1), axis=0)
            all_color_a2 = np.concatenate((color_a2_dummy, level_1_color_a2), axis=0)
            all_color_c2 = np.concatenate((color_c2_dummy, level_1_color_c2), axis=0)
            all_color_a3 = np.concatenate((color_a3_dummy, level_1_color_a3), axis=0)
            all_color_c3 = np.concatenate((color_c3_dummy, level_1_color_c3), axis=0)
            ...
            all_color_a1 = color_a1_dummy
            all_color_c1 = color_c1_dummy
            all_color_a2 = color_a2_dummy
            all_color_c2 = color_c2_dummy
            all_color_a3 = color_a3_dummy
            all_color_c3 = color_c3_dummy
    ...
    def load_ply(self, path):
        ...
        color_a1_names = [p.name for p in ply_data.elements[0].properties if p.name.startswith("color_a1")]
        color_a1 = np.zeros((xyz.shape[0], len(color_a1_names)))
        for idx, attr_name in enumerate(color_a1_names):
            color_a1[:, idx] = np.asarray(ply_data.elements[0][attr_name])

        color_c1_names = [p.name for p in ply_data.elements[0].properties if p.name.startswith("color_c1")]
        color_c1 = np.zeros((xyz.shape[0], len(color_c1_names)))
        for idx, attr_name in enumerate(color_c1_names):
            color_c1[:, idx] = np.asarray(ply_data.elements[0][attr_name])

        color_a2_names = [p.name for p in ply_data.elements[0].properties if p.name.startswith("color_a2")]
        color_a2 = np.zeros((xyz.shape[0], len(color_a2_names)))
        for idx, attr_name in enumerate(color_a2_names):
            color_a2[:, idx] = np.asarray(ply_data.elements[0][attr_name])

        color_c2_names = [p.name for p in ply_data.elements[0].properties if p.name.startswith("color_c2")]
        color_c2 = np.zeros((xyz.shape[0], len(color_c2_names)))
        for idx, attr_name in enumerate(color_c2_names):
            color_c2[:, idx] = np.asarray(ply_data.elements[0][attr_name])

        color_a3_names = [p.name for p in ply_data.elements[0].properties if p.name.startswith("color_a3")]
        color_a3 = np.zeros((xyz.shape[0], len(color_a3_names)))
        for idx, attr_name in enumerate(color_a3_names):
            color_a3[:, idx] = np.asarray(ply_data.elements[0][attr_name])

        color_c3_names = [p.name for p in ply_data.elements[0].properties if p.name.startswith("color_c3")]
        color_c3 = np.zeros((xyz.shape[0], len(color_c3_names)))
        for idx, attr_name in enumerate(color_c3_names):
            color_c3[:, idx] = np.asarray(ply_data.elements[0][attr_name])
        ...
        self.color_a1 = torch.tensor(color_a1[level_1_idx], dtype=torch.float, device="cuda")
        self.color_c1 = torch.tensor(color_c1[level_1_idx], dtype=torch.float, device="cuda")
        self.color_a2 = torch.tensor(color_a2[level_1_idx], dtype=torch.float, device="cuda")
        self.color_c2 = torch.tensor(color_c2[level_1_idx], dtype=torch.float, device="cuda")
        self.color_a3 = torch.tensor(color_a3[level_1_idx], dtype=torch.float, device="cuda")
        self.color_c3 = torch.tensor(color_c3[level_1_idx], dtype=torch.float, device="cuda")
        ...
        if np.sum(level_1_idx) > 0:
            ...
            # self._level_1_features_rest = nn.Parameter(
            #     torch.tensor(features_extra[level_1_idx], dtype=torch.float, device="cuda").requires_grad_(True)
            # )
            self._level_1_color_a1 = nn.Parameter(
                torch.tensor(color_a1[level_1_idx], dtype=torch.float, device="cuda").requires_grad_(True)
            )
            self._level_1_color_c1 = nn.Parameter(
                torch.tensor(color_c1[level_1_idx], dtype=torch.float, device="cuda").requires_grad_(True)
            )
            self._level_1_color_a2 = nn.Parameter(
                torch.tensor(color_a2[level_1_idx], dtype=torch.float, device="cuda").requires_grad_(True)
            )
            self._level_1_color_c2 = nn.Parameter(
                torch.tensor(color_c2[level_1_idx], dtype=torch.float, device="cuda").requires_grad_(True)
            )
            self._level_1_color_a3 = nn.Parameter(
                torch.tensor(color_a3[level_1_idx], dtype=torch.float, device="cuda").requires_grad_(True)
            )
    ...
    def prune_points_level_1(self, mask):
        ...
        self._level_1_color_a1 = optimizable_tensors["color_a1"]
        self._level_1_color_c1 = optimizable_tensors["color_c1"]
        self._level_1_color_a2 = optimizable_tensors["color_a2"]
        self._level_1_color_c2 = optimizable_tensors["color_c2"]
        self._level_1_color_a3 = optimizable_tensors["color_a3"]
        self._level_1_color_c3 = optimizable_tensors["color_c3"]
    ...
    def densification_postfix_level_1(
        ...
        d = {
            ...
            # "motion": new_motion,
            "omega": new_omega,
            "color_a1": new_color_a1,
            "color_c1": new_color_c1,
            "color_a2": new_color_a2,
            "color_c2": new_color_c2,
            "color_a3": new_color_a3,
            "color_c3": new_color_c3,
        }
    ...
    def densify_and_split_level_1(self, grads, grad_threshold, scene_extent, max_timestamp=1.0, N=2):
        ...
        new_color_a1 = self._level_1_color_a1[selected_pts_mask].repeat(N, 1)
        new_color_c1 = self._level_1_color_c1[selected_pts_mask].repeat(N, 1)
        new_color_a2 = self._level_1_color_a2[selected_pts_mask].repeat(N, 1)
        new_color_c2 = self._level_1_color_c2[selected_pts_mask].repeat(N, 1)
        new_color_a3 = self._level_1_color_a3[selected_pts_mask].repeat(N, 1)
        new_color_c3 = self._level_1_color_c3[selected_pts_mask].repeat(N, 1)
    ...
    def densify_and_clone_level_1(self, grads, grad_threshold, scene_extent, max_timestamp=1.0):
        ...
        new_color_a1 = self._level_1_color_a1[selected_pts_mask]
        new_color_c1 = self._level_1_color_c1[selected_pts_mask]
        new_color_a2 = self._level_1_color_a2[selected_pts_mask]
        new_color_c2 = self._level_1_color_c2[selected_pts_mask]
        new_color_a3 = self._level_1_color_a3[selected_pts_mask]
        new_color_c3 = self._level_1_color_c3[selected_pts_mask]
        self.densification_postfix_level_1(
            new_parent_idx,
            new_features_dc,
            new_opacities,
