...
int loadPly(const char* filename,
	...
	sibr::Vector3f& minn,
	sibr::Vector3f& max_x,
	std::vector<float>& trbf_center,
	std::vector<float>& trbf_scale,
	...
	std::vector<Color<3>>& colors_precomp)
{
	...
	scales.resize(count);
	rot.resize(count);
	trbf_center.resize(count);
	trbf_scale.resize(count);
	...
	{   
		...
		max_x = max_x.cwiseMax(points[i].pos );
	...
	for (int i = 0; i < count; i++)
	{
		...
		sibr::Vector3f rel = (points[i].pos - minn).array() / (max_x - minn).array();
	...
	for (int k = 0; k < count; k++)
	{
		...
		trbf_center[k] = points[i].trbf_center;
		trbf_scale[k] = exp(points[i].trbf_scale); //precomputed
	...
	const sibr::Vector3f& max_x)
{
	...
	for (int i = 0; i < pos.size(); i++)
	{
		if (pos[i].x() < minn.x() || pos[i].y() < minn.y() || pos[i].z() < minn.z() ||
			pos[i].x() > max_x.x() || pos[i].y() > max_x.y() || pos[i].z() > max_x.z())
			continue;
		count++;
	}
	...
	sibr::ViewBase(render_w, render_h)
{
	...
	if (sh_degree == 1)
	{
		count = loadPly<1>(file, pos, opacity, scale, rot,rott, rotdummy,_scenemin, _scenemax, trbf_center, trbf_scale, motion, means3Ddummy, opacitiesdummy, colors_precomp);
	}
	else if (sh_degree == 2)
	{
		count = loadPly<2>(file, pos, opacity, scale, rot,rott, rotdummy, _scenemin, _scenemax, trbf_center, trbf_scale, motion, means3Ddummy, opacitiesdummy, colors_precomp);
	}
	else if (sh_degree == 3)
	{
		count = loadPly<3>(file, pos, opacity, scale, rot, rott, rotdummy, _scenemin, _scenemax, trbf_center, trbf_scale, motion, means3Ddummy, opacitiesdummy, colors_precomp);
	}
	...
	CUDA_SAFE_CALL_ALWAYS(cudaMalloc((void**)&trbf_center_cuda, sizeof(float) * P));
	CUDA_SAFE_CALL_ALWAYS(cudaMemcpy(trbf_center_cuda, trbf_center.data(), sizeof(float) * P, cudaMemcpyHostToDevice));

	CUDA_SAFE_CALL_ALWAYS(cudaMalloc((void**)&trbf_scale_cuda, sizeof(float) * P));
	CUDA_SAFE_CALL_ALWAYS(cudaMemcpy(trbf_scale_cuda, trbf_scale.data(), sizeof(float) * P, cudaMemcpyHostToDevice));
...
sibr::GaussianView::~GaussianView()
{
	...
	cudaFree(trbf_center_cuda);
	cudaFree(trbf_scale_cuda);
