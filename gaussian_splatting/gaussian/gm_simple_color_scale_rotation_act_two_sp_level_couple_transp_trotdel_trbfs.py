...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int, rgb_function="rgbv1"):
        ...
        self._delta_trot_radius = torch.empty(0)  # dummy
        self._delta_trot_angle_vel = torch.empty(0)  # dummy
        ...
        self._level_1_delta_trot_center = torch.empty(0)
        self._level_1_delta_trot_radius = torch.empty(0)
        self._level_1_delta_trot_alpha = torch.empty(0)
        self._level_1_delta_trot_angle_vel = torch.empty(0)
        self._level_1_delta_trot_beta = torch.empty(0)
    ...
    def get_level_1_delta_trot_radius(self, timestamp):
        level_1_delta_trot_radius = (
            self._level_1_delta_trot_radius
            + (timestamp - self._level_1_delta_trot_center) * self._level_1_delta_trot_alpha
        )
        level_1_delta_trot_radius = torch.relu(level_1_delta_trot_radius)
        return level_1_delta_trot_radius

    def get_level_1_delta_trot_angle_vel(self, timestamp):
        level_1_delta_trot_angle_vel = (
            self._level_1_delta_trot_angle_vel
            + (timestamp - self._level_1_delta_trot_center) * self._level_1_delta_trot_beta
        )
        return level_1_delta_trot_angle_vel
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        self._delta_trot_center = torch.zeros((fused_point_cloud.shape[0], 1), device="cuda")
        self._delta_trot_radius = torch.zeros((fused_point_cloud.shape[0], 3), device="cuda")
        self._delta_trot_alpha = torch.ones((fused_point_cloud.shape[0], 3), device="cuda")
        self._delta_trot_angle_vel = torch.zeros((fused_point_cloud.shape[0], 3), device="cuda")
        self._delta_trot_beta = torch.ones((fused_point_cloud.shape[0], 3), device="cuda")
        print(f"self._delta_trot_center inited {self._delta_trot_center}")
        print(f"self._delta_trot_radius inited {self._delta_trot_radius}")
        print(f"self._delta_trot_alpha inited {self._delta_trot_alpha}")
    ...
    def create_another_level(
        ...
        **kwargs,
        ...
        level_1_delta_trot_center = level_1_total_times
        self._level_1_delta_trot_center = nn.Parameter(level_1_delta_trot_center.requires_grad_(True))
        print(f"self._level_1_delta_trot_center inited {self._level_1_delta_trot_center}")

        level_1_scales_mean = torch.exp(torch.mean(level_1_scales, dim=1, keepdim=True))
        level_1_delta_trot_radius = (
            torch.zeros((level_1_total_parent_idx.shape[0], 3), device="cuda")
            + new_pts_init_delta_rot_radius_scale * level_1_scales_mean
        )
        self._level_1_delta_trot_radius = nn.Parameter(level_1_delta_trot_radius.requires_grad_(True))
        print(f"self._level_1_delta_trot_radius inited {self._level_1_delta_trot_radius}")

        level_1_delta_trot_alpha = torch.ones((level_1_total_parent_idx.shape[0], 3), device="cuda")
        self._level_1_delta_trot_alpha = nn.Parameter(level_1_delta_trot_alpha.requires_grad_(True))
        print(f"self._level_1_delta_trot_alpha inited {self._level_1_delta_trot_alpha}")

        if new_pts_init_delta_rot_angle_vel_rand is not None:
            level_1_delta_trot_angle_vel = (
                torch.rand((level_1_total_parent_idx.shape[0], 3), device="cuda")
                * new_pts_init_delta_rot_angle_vel_rand
            )
        else:
            level_1_delta_trot_angle_vel = torch.zeros((level_1_total_parent_idx.shape[0], 3), device="cuda")
        self._level_1_delta_trot_angle_vel = nn.Parameter(level_1_delta_trot_angle_vel.requires_grad_(True))
    ...
    def cache_level_1_gradient(self):
        ...
        self._level_1_delta_trot_center_grad += self._level_1_delta_trot_center.grad.clone()
        self._level_1_delta_trot_radius_grad += self._level_1_delta_trot_radius.grad.clone()
        self._level_1_delta_trot_alpha_grad += self._level_1_delta_trot_alpha.grad.clone()
        self._level_1_delta_trot_angle_vel_grad += self._level_1_delta_trot_angle_vel.grad.clone()
        self._level_1_delta_trot_beta_grad += self._level_1_delta_trot_beta.grad.clone()
    ...
    def set_level_1_batch_gradient(self, batch_size):
        ...
        self._level_1_delta_trot_center.grad = self._level_1_delta_trot_center_grad * ratio
        self._level_1_delta_trot_radius.grad = self._level_1_delta_trot_radius_grad * ratio
        self._level_1_delta_trot_alpha.grad = self._level_1_delta_trot_alpha_grad * ratio
        self._level_1_delta_trot_angle_vel.grad = self._level_1_delta_trot_angle_vel_grad * ratio
        self._level_1_delta_trot_beta.grad = self._level_1_delta_trot_beta_grad * ratio
    ...
    def level_1_training_setup(self, training_args):
        ...
        l = [
            ...
            # },
            {
                "params": [self._level_1_delta_trot_center],
                "lr": training_args.level_1_delta_trot_center_lr,
                "name": "delta_trot_center",
            },
            {
                "params": [self._level_1_delta_trot_radius],
                "lr": training_args.level_1_delta_trot_radius_lr,
                "name": "delta_trot_radius",
            },
            {
                "params": [self._level_1_delta_trot_angle_vel],
                "lr": training_args.level_1_delta_trot_angle_vel_lr,
                "name": "delta_trot_angle_vel",
            },
            {
                "params": [self._level_1_delta_trot_alpha],
                "lr": training_args.level_1_delta_trot_alpha_lr,
                "name": "delta_trot_alpha",
            },
            {
                "params": [self._level_1_delta_trot_beta],
                "lr": training_args.level_1_delta_trot_beta_lr,
                "name": "delta_trot_beta",
            },
        ]
    ...
    def construct_list_of_attributes(self):
        ...
        l.append("delta_trot_center")
        for i in range(self._delta_trot_radius.shape[1]):
            l.append(f"delta_trot_radius_{i}")

        for i in range(self._delta_trot_alpha.shape[1]):
            l.append(f"delta_trot_alpha_{i}")

        for i in range(self._delta_trot_angle_vel.shape[1]):
            l.append(f"delta_trot_angle_vel_{i}")

        for i in range(self._delta_trot_beta.shape[1]):
            l.append(f"delta_trot_beta_{i}")
    ...
    def save_ply(self, path):
        ...
        delta_trot_center_dummy = np.zeros((xyz.shape[0], 1))
        delta_trot_radius_dummy = np.zeros((xyz.shape[0], 3))
        delta_trot_alpha_dummy = np.ones((xyz.shape[0], 3))
        delta_trot_angle_vel_dummy = np.zeros((xyz.shape[0], 3))
        delta_trot_beta_dummy = np.ones((xyz.shape[0], 3))
        ...
        if self._level_1_parent_idx.shape[0] > 0:
            ...
            level_1_delta_trot_center = self._level_1_delta_trot_center.detach().cpu().numpy()
            level_1_delta_trot_radius = self._level_1_delta_trot_radius.detach().cpu().numpy()
            level_1_delta_trot_alpha = self._level_1_delta_trot_alpha.detach().cpu().numpy()
            level_1_delta_trot_angle_vel = self._level_1_delta_trot_angle_vel.detach().cpu().numpy()
            level_1_delta_trot_beta = self._level_1_delta_trot_beta.detach().cpu().numpy()
            ...
            all_delta_trot_center = np.concatenate((delta_trot_center_dummy, level_1_delta_trot_center), axis=0)
            all_delta_trot_radius = np.concatenate((delta_trot_radius_dummy, level_1_delta_trot_radius), axis=0)
            all_delta_trot_alpha = np.concatenate((delta_trot_alpha_dummy, level_1_delta_trot_alpha), axis=0)
            all_delta_trot_angle_vel = np.concatenate(
                (delta_trot_angle_vel_dummy, level_1_delta_trot_angle_vel), axis=0
            )
            all_delta_trot_beta = np.concatenate((delta_trot_beta_dummy, level_1_delta_trot_beta), axis=0)
            ...
            all_delta_trot_center = delta_trot_center_dummy
            all_delta_trot_radius = delta_trot_radius_dummy
            all_delta_trot_alpha = delta_trot_alpha_dummy
            all_delta_trot_angle_vel = delta_trot_angle_vel_dummy
            all_delta_trot_beta = delta_trot_beta_dummy
        ...
        attributes = np.concatenate(
            (
                ...
                all_level,
                all_parent_idx,
                all_delta_trot_center,
                all_delta_trot_radius,
                all_delta_trot_alpha,
                all_delta_trot_angle_vel,
                all_delta_trot_beta,
            ),
            axis=1,
        )
    ...
    def load_ply(self, path):
        ...
        self._delta_trot_center = torch.tensor(delta_trot_center[level_0_idx], dtype=torch.float, device="cuda")
        self._delta_trot_radius = torch.tensor(delta_trot_radius[level_0_idx], dtype=torch.float, device="cuda")
        self._delta_trot_alpha = torch.tensor(delta_trot_alpha[level_0_idx], dtype=torch.float, device="cuda")
        self._delta_trot_angle_vel = torch.tensor(delta_trot_angle_vel[level_0_idx], dtype=torch.float, device="cuda")
        self._delta_trot_beta = torch.tensor(delta_trot_beta[level_0_idx], dtype=torch.float, device="cuda")
        ...
        if np.sum(level_1_idx) > 0:
            ...
            self._level_1_delta_trot_center = nn.Parameter(
                torch.tensor(delta_trot_center[level_1_idx], dtype=torch.float, device="cuda").requires_grad_(True)
            )
            self._level_1_delta_trot_radius = nn.Parameter(
                torch.tensor(delta_trot_radius[level_1_idx], dtype=torch.float, device="cuda").requires_grad_(True)
            )
            self._level_1_delta_trot_alpha = nn.Parameter(
                torch.tensor(delta_trot_alpha[level_1_idx], dtype=torch.float, device="cuda").requires_grad_(True)
            )
            self._level_1_delta_trot_angle_vel = nn.Parameter(
                torch.tensor(delta_trot_angle_vel[level_1_idx], dtype=torch.float, device="cuda").requires_grad_(True)
            )
            self._level_1_delta_trot_beta = nn.Parameter(
                torch.tensor(delta_trot_beta[level_1_idx], dtype=torch.float, device="cuda").requires_grad_(True)
            )
    ...
    def prune_points_level_1(self, mask):
        ...
        self._level_1_delta_trot_center = optimizable_tensors["delta_trot_center"]
        self._level_1_delta_trot_radius = optimizable_tensors["delta_trot_radius"]
        self._level_1_delta_trot_alpha = optimizable_tensors["delta_trot_alpha"]
        self._level_1_delta_trot_angle_vel = optimizable_tensors["delta_trot_angle_vel"]
        self._level_1_delta_trot_beta = optimizable_tensors["delta_trot_beta"]
    ...
    def densification_postfix_level_1(
        ...
        # new_motion,
        new_omega,
        new_delta_trot_center,
        new_delta_trot_radius,
        new_delta_trot_alpha,
        new_delta_trot_angle_vel,
        new_delta_trot_beta,
        dummy=None,
    ):
        d = {
            ...
            # "motion": new_motion,
            "omega": new_omega,
            "delta_trot_center": new_delta_trot_center,
            "delta_trot_radius": new_delta_trot_radius,
            "delta_trot_alpha": new_delta_trot_alpha,
            "delta_trot_angle_vel": new_delta_trot_angle_vel,
            "delta_trot_beta": new_delta_trot_beta,
        }
    ...
    def densify_and_split_level_1(self, grads, grad_threshold, scene_extent, max_timestamp=1.0, N=2):
        ...
        new_delta_trot_center = self._level_1_delta_trot_center[selected_pts_mask].repeat(N, 1)
        new_delta_trot_center = torch.rand_like(new_trbf_center) * max_timestamp
        new_delta_trot_radius = self._level_1_delta_trot_radius[selected_pts_mask].repeat(N, 1)
        new_delta_trot_alpha = self._level_1_delta_trot_alpha[selected_pts_mask].repeat(N, 1)
        new_delta_trot_angle_vel = self._level_1_delta_trot_angle_vel[selected_pts_mask].repeat(N, 1)
        new_delta_trot_beta = self._level_1_delta_trot_beta[selected_pts_mask].repeat(N, 1)
    ...
    def densify_and_clone_level_1(self, grads, grad_threshold, scene_extent, max_timestamp=1.0):
        ...
        new_delta_trot_alpha = self._level_1_delta_trot_alpha[selected_pts_mask]
        new_delta_trot_angle_vel = self._level_1_delta_trot_angle_vel[selected_pts_mask]
        new_delta_trot_beta = self._level_1_delta_trot_beta[selected_pts_mask]
        self.densification_postfix_level_1(
            new_parent_idx,
            new_features_dc,
            new_opacities,
